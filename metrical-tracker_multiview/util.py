...
def get_gaussian_pyramid(levels, input, kernel_size, sigma):
    ...
    for k, level in enumerate(reversed(levels)):
        ...
        images = torch.cat([F.interpolate(img.unsqueeze(0), size, mode='bilinear', align_corners=False) for img in images], dim=0)
        images = torch.cat([gaussian_blur(img.unsqueeze(0), [kernel_size, kernel_size], sigma=[sigma, sigma] if sigma is not None else None) for img in images], dim=0)
...
def to_image(img):
    img = (img.transpose(0, 2, 3, 1) * 255)[:, :, :, [2, 1, 0]]
...
def images_to_video(path, fps=25, src='video', video_format='mp4v'):
    img_array = []
    ...
    if len(img_array) > 0:
        out = cv2.VideoWriter(f'{path}/video.mp4', cv2.VideoWriter_fourcc(*video_format), fps, size)
