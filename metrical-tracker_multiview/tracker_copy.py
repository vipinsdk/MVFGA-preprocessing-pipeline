...
class Tracker(object):
    ...
    def rotvec_to_rotation6D(self, rot_vec):
        return matrix_to_rotation_6d(torch.from_numpy(R.from_rotvec(rot_vec).as_matrix()).float())
    ...
    def load_checkpoint(self, idx=-1):
        ...
        self.rotation = nn.Parameter(torch.from_numpy(R.from_rotvec(flame_params['rotation']).as_matrix()).float().to(self.device))
        ...
        self.shape = nn.Parameter(torch.from_numpy(flame_params['shape']).unsqueeze(0).to(self.device))
        ...
        self.eyes = nn.Parameter(torch.cat([self.rotvec_to_rotation6D(flame_params['eyes_pose'][:, :3]), self.rotvec_to_rotation6D(flame_params['eyes_pose'][:, 3:])], dim=1)).to(self.device)
        ...
        self.jaw = nn.Parameter(self.rotvec_to_rotation6D(flame_params['jaw_pose'])).to(self.device)
    ...
    def rotation6D_to_rotvec(self, rotation6D):
        return R.from_matrix(rotation_6d_to_matrix(rotation6D).cpu().numpy()).as_rotvec()

    def save_checkpoint(self, frame_id):
        opencv = opencv_from_cameras_projection(self.cameras, self.image_size)
        jaw = self.rotation6D_to_rotvec(self.jaw.clone().detach())
        rotation = R.from_matrix(self.rotation.clone().cpu().numpy()).as_rotvec()
        right_eye, left_eye = self.eyes[:, :6].clone().detach(), self.eyes[:, 6:].clone().detach()
        right_eye = self.rotation6D_to_rotvec(right_eye)
        left_eye = self.rotation6D_to_rotvec(left_eye)
        eyes = np.concatenate([right_eye, left_eye], axis=1)
        neck = np.ones((1, 3), dtype=np.float32)

        frame = {
            'flame': {
                'expr': self.expr.clone().detach().cpu().numpy(),
                'shape': self.shape.squeeze().clone().detach().cpu().numpy(),
        ...
        np.savez(f'{self.checkpoint_folder}/{frame_id}.npz', **frame['flame'])
    ...
    def optimize_camera(self, batch, steps=5000):
        ...
        params = [{'params': [self.translation], 'lr': 0.05, 'name': ['translation']},
    ...
    def checkpoint(self, batch, visualizations=[[View.GROUND_TRUTH, View.LANDMARKS, View.HEATMAP], [View.COLOR_OVERLAY, View.SHAPE_OVERLAY, View.SHAPE, View.SHAPE_MERGE]], frame_dst='/video', save=True, dump_directly=False):
        ...
        with torch.no_grad():
            ...
            # vertices[0] = vertices[0] @ self.R_1.squeeze() + self.T_1.squeeze()
            ...
            for views in visualizations:
                ...
                for view in views:
                    ...
                    if view == View.SHAPE_MERGE:
                        ...
                        # blend1 = images * mask[:, None, :, :] + background_tensor * (1 - mask[:, None, :, :])
                        blend = images.cpu().numpy() * 0.5 + shape.cpu().numpy() * 0.5
                    #    blend2 = blend * mask[:, None, :, :].cpu().numpy()
                        row.append(blend)
            ...
            mask = self.get_mask_tilted_line(verts_ndc)
            background_tensor = torch.tensor([255, 255, 255])[:, None, None]
            transform = T.ToPILImage()
            for j in range(self.num_cameras):
                rgb_path = self.config.flame_param_path + "/" + f"{self.frame:05d}_{j:02d}.png"
                image= Image.open(rgb_path).convert("RGB")
                image = F_t.to_tensor(image).to(self.device)
                # background_tensor = background_tensor.expand_as(image).byte().to(self.device)
                image = image * mask[j][None, :, :] #+ background_tensor * (1 - mask[j][None, :, :])
                rgb_path = rgb_path.replace('images', 'images_')
                if not os.path.exists(os.path.dirname(rgb_path)):
                    os.makedirs(os.path.dirname(rgb_path))
                img = transform(image.cpu())
                img.save(rgb_path)

    def dataset_export(self, batch):
        for i in list(range(self.frame, len(self.dataset))):
            batch = self.dataset[i]
            if type(batch) is torch.Tensor:
                continue
            batch = self.to_cuda(batch)
            images, _, _, _, _ = self.parse_batch(batch)
            ...
            with torch.no_grad():
                ...
                flame_params = batch['flame_param']
                rotation = nn.Parameter(torch.from_numpy(R.from_rotvec(flame_params['rotation']).as_matrix()).float().to(self.device))
                translation = nn.Parameter(torch.from_numpy(flame_params['translation']).to(self.device))
                expr = nn.Parameter(torch.from_numpy(flame_params['expr']).to(self.device))
                shape = nn.Parameter(torch.from_numpy(flame_params['shape']).unsqueeze(0).to(self.device))
                eyes = nn.Parameter(torch.cat([self.rotvec_to_rotation6D(flame_params['eyes_pose'][:, :3]), self.rotvec_to_rotation6D(flame_params['eyes_pose'][:, 3:])], dim=1)).to(self.device)
                eyelids = nn.Parameter(torch.from_numpy(flame_params['eyelids']).to(self.device))
                jaw = nn.Parameter(self.rotvec_to_rotation6D(flame_params['jaw_pose'])).to(self.device)
                ...
                shape = self.render_shape(vertices_n, white=False)
                mask = self.get_mask_tilted_line(verts_ndc)
                blend = images * mask[:, None, :, :]
                final_views.append([shape.cpu().numpy()])
                # VIDEO
                final_views = util.merge_views(final_views)
                concatenated_views = np.concatenate(final_views, axis=1)
                frame_id = str(i).zfill(5)

                savefolder = self.save_folder + self.actor_name + '/video'
                Path(savefolder).mkdir(parents=True, exist_ok=True)
                cv2.imwrite('{}/{}.jpg'.format(savefolder, frame_id), concatenated_views)
                # # save the images
                # for j in range(self.num_cameras):
                #     image = shape[j].cpu().numpy().transpose(1, 2, 0)
                #     rgb_path = self.config.flame_param_path + "/" + f"{i:05d}_{j:02d}.jpg"
                #     rgb_path = rgb_path.replace('flame_param', 'images_')
                #     if not os.path.exists(os.path.dirname(rgb_path)):
                #         os.makedirs(os.path.dirname(rgb_path))
    ...
    def optimize_frame(self, batch):
        ...
        self.checkpoint(batch, visualizations=[[View.GROUND_TRUTH, View.COLOR_OVERLAY, View.LANDMARKS, View.SHAPE_MERGE, View.SHAPE]])
    ...
    def initialize_tracking(self):
        ...
        for i, j in enumerate(keyframes):
            ...
            self.checkpoint(batch, visualizations=[[View.GROUND_TRUTH, View.COLOR_OVERLAY, View.LANDMARKS, View.SHAPE_MERGE, View.SHAPE]], frame_dst='/initialization')
            self.frame += 1
