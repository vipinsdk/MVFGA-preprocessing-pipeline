...
from datasets.generate_dataset import GeneratorDataset, VideoDataset
...
class Tracker(object):
    ...
    def load_checkpoint(self, idx=-1):
        ...
        self.rotation = nn.Parameter(torch.from_numpy(R.from_rotvec(flame_params['rotation']).as_matrix()).to(self.device))
        ...
        self.eyes = nn.Parameter(torch.cat([self.rotvec_to_rotation6D(flame_params['eyes'][:, :3]), self.rotvec_to_rotation6D(flame_params['eyes'][:, 3:])], dim=1)).to(self.device)
        ...
        self.jaw = nn.Parameter(self.rotvec_to_rotation6D(flame_params['jaw'])).to(self.device)
        self.neck = nn.Parameter(self.rotvec_to_rotation6D(flame_params['neck'])).to(self.device)
    ...
    def save_checkpoint(self, frame_id):
        ...
        neck = self.rotation6D_to_rotvec(self.neck.clone().detach())
    ...
    def create_parameters(self):
        ...
        # self.neck = nn.Parameter(matrix_to_rotation_6d(I))
        self.eyes = nn.Parameter(torch.zeros(bz, 12).float().to(self.device))
        self.jaw = nn.Parameter(torch.zeros(bz, 6).float().to(self.device))
        self.neck = nn.Parameter(torch.zeros(bz, 6).float().to(self.device))
    ...
    def optimize_color(self, batch, pyramid, params_func, pho_weight_func, reg_from_prev=False):
        ...
        for k, level in enumerate(pyramid):
            ...
            for p in range(iters):
                ...
                losses['reg/neck'] = torch.sum((I6D - neck) ** 2) * self.config.w_neck
            ...
            for views in visualizations:
                ...
                for view in views:
                    ...
                    if view == View.SHAPE_MERGE:
                        shape = self.render_shape(vertices_n, white=False).cpu().numpy()
                        blend = images.cpu().numpy() * 0.5 + shape * 0.5
                        row.append(blend)
