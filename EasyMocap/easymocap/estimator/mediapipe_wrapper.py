...
class Detector:
    ...
    def __init__(self, nViews, to_openpose, model_type, show=False, **cfg) -> None:
        ...
        self.img_save = '/netscratch/jeetmal/videos/sitting/img_save/'
        self.show = True
        if self.to_openpose:
            self.NUM_BODY = 25
    ...
    def vis(self, image, annots, nv=0):
        ...
        if 'face2d' in annots.keys():
            kpts = annots['face2d']
            plot_keypoints(image, kpts, 0, CONFIG['mpface'], use_limb_color=True)
            if len(kpts) > 468:
    ...
    def __call__(self, images):
        ...
        for nv, image_ in enumerate(images):
            ...
            if self.show:
                res_img = self.vis(image_, annots, nv)
            annots_all.append(annots)
            # results.face_landmarks
        return annots_all, res_img

def extract_2d(image_root, annot_root, pose_out, config, mode='holistic', pose_save=False):
    ...
    for imgname in tqdm(imgnames, desc='{:10s}'.format(os.path.basename(annot_root))):
        ...
        annotname = join(annot_root, base.replace(ext, '') +'.json')
        image = cv2.imread(imgname)
        annots_all, res_img = detector([image]) 
        annots = annots_all[0]

        # save the pose output
        if pose_save:
            poseoutname = join(pose_out, base)
            if not os.path.exists(os.path.dirname(poseoutname)):
                os.makedirs(os.path.dirname(poseoutname))
            cv2.imwrite(poseoutname, res_img)
