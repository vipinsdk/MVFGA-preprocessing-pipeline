...
def convert_from_openpose(src, dst, image_root, ext):
    ...
    for inp in tqdm(inputlist, desc='{:10s}'.format(os.path.basename(dst))):
        ...
        base = inp.replace('_keypoints.json', '').split('_')[1][6:]
        annotname = join(dst, base+'.json')
        imgname = inp.split('_')[1][:6] + ext
        imgname = join(image_root, imgname)
...
if __name__ == '__main__':
  src = '/netscratch/jeetmal/videos/six_cam/cropped/openpose/6'
  dst = '/netscratch/jeetmal/videos/six_cam/cropped/openpose_2d/6'
  image_root = '/netscratch/jeetmal/videos/six_cam/cropped/images/6'
  ext = '.jpg'
  convert_from_openpose(src, dst, image_root, ext)