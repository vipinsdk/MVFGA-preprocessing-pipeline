...
def read_extri(extri_name):
    ...
    for key in camnames:
        ...
        cam['R'] = cv2.Rodrigues(cam['Rvec'])[0]
...
def write_extri(extri_name, cameras, image_id=0):
...
def create_camera_json(cameras, cameras_json_out):
    import json
    cameras_json = {
        'intrinsics': {},
        'world_2_cam': {},
    ...
    for key, val in cameras.items():
        cameras_json['intrinsics'][key] = val['K'].tolist()
        cameras_json['world_2_cam'][key] = np.vstack((val['RT'], [0, 0, 0, 1])).tolist()
        cameras_json['dist'][key] = val['dist'].tolist()
    with open(cameras_json_out, 'w') as f:
...
def read_camera(intri_name, extri_name, cam_names=[]):
    ...
    cameras_json_out = intri_name.replace('intri.yml', 'camera_params.json')
    create_camera_json(cams, cameras_json_out)
