...
# MP_LANDMARKS = [276, 282, 283, 285, 293, 295, 296, 300, 334, 336,  46,  52,  53,  55,  63,  65,  66,  70, 105, 107, 249, 263, 362, 373, 374, 380, 381, 382, 384, 385, 386, 387, 388, 390, 398, 466, 7,  33, 133, 144, 145, 153, 154, 155, 157, 158, 159, 160, 161, 163, 173, 246, 168,   6, 197, 195,   5,   4, 129,  98,  97,   2, 326, 327, 358,   0,  13,  14,  17,  37,  39,  40, 61,  78,  80,  81,  82,  84,  87,  88,  91,  95, 146, 178, 181, 185, 191, 267, 269, 270, 291, 308, 310, 311, 312, 314, 317, 318, 321, 324, 375, 402, 405, 409, 415]
# MP_LANDMARKS = [ 0,  267, 270, 291, 17, 84, 314, 91, 321, 78, 82, 13, 312,308 ,317, 14, 87 ]
MP_LANDMARKS = [46, 53, 52, 65,55,285, 295, 282, 283, 276, 168, 197, 5, 1, 98, 97, 2, 326, 327, 33, 160, 158, 133, 153, 144,  362, 385, 386, 263, 374, 380, 61, 40, 37, 0, 267, 270, 291, 321, 314, 17, 84, 91, 78, 82, 13, 312, 308, 317, 14, 87]
KEYPOINTS_ZERO = [10, 11, 13, 14, 19, 20, 21, 22, 23, 24]
...
def read_annot(annotname, mode='body25'):
    ...
    for i in range(len(data)):
        ...
        for key in ['bbox', 'keypoints', 
            ...
            # setting the keypoints for the lower body to zero
            # if key == 'keypoints':
            #     data[i][key][KEYPOINTS_ZERO, :] = 0
            if key == 'face2d':
                # TODO: Make parameters, 17 is the offset for the eye brows,
                # etc. 51 is the total number of FLAME compatible landmarks
                if data[i][key].shape[0] == 468:
                    # print('Warning: {} has more than 51 landmarks for face'.format(annotname))
                    data[i][key] = data[i][key][MP_LANDMARKS, :]
                else:
