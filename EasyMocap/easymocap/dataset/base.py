...
from ..mytools.camera import change_camera_coord_convention, align_cameras_to_axes
...
def load_cameras(path):
    ...
    print(f"load cameras: ",intri_name, extri_name)
...
class MVBase:
    ...
    def __init__(self, root, cams=[], out=None, config={}, 
        ...
        # self.align_cameras_to_axes: bool = True
        # self.camera_coord_conversion: str = 'opencv->opengl'
        # self.target_extrinsic_type: Literal['w2c', 'c2w'] = 'c2w'
    ...
    def load_camera_params(self):
        load_path = Path(self.root) / "camera_params.json"
        ...
        for i, camera_id in enumerate(self.camera_ids):
            self.camera_params[camera_id] = {"intrinsic": K[i], "extrinsic": w2c[i], "dist": dist[i]}
        ...
        for cam in self.cams:
            ...
            if self.has2d:
                ...
                if self.manopath is not None:
                    manopath = self.manopath + '/' + f'{cam}_{index:05d}.npz'
                    if os.path.exists(manopath):
    ...
    def write(self):
        ...
        for nf in tqdm.tqdm(range(numFrames), desc='writing json', total=numFrames):
            image, _ = self.__getitem__(nf)
            for camera_index, camera_id in enumerate(self.cams[:15]):
                ...
                intrinsic = camera['intrinsic'].double().numpy()
